...
public class MozLevelDesigner : EditorWindow {
	...
	private void GridSetupPanel(){
		...
		if( GUILayout.Button( "Edit grid" ) ){ EditGrid(); }
		if( GUILayout.Button( "Clean grid" ) ){ CleanGrid(); }
		
		EditorGUILayout.Separator();
		
		if( GUILayout.Button( "Create pieces" ) ){ CreatePieces(); }
		if( GUILayout.Button( "Destroy pieces" ) ){ DestroyPieces(); }
	}
