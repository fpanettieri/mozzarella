...
public class GridInput : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if (Input.GetMouseButtonUp (0) && InsideGrid (Input.mousePosition)) {
			...
			timeline.Add( new PieceSpawnEvent (machine.now, cell.x, cell.y, queue.Next()) );
			timeline.Add( new PieceSpawnEvent (machine.now, (cell.x + 1) % grid.columns, cell.y, queue.Next()) );
		}
	}
